...
router.get('/', async (req, res) => {
  try {
    const t = today();
    const stats = {
      deliveries: (await db.getAsync('SELECT COUNT(*) c FROM deliveries WHERE date=?', [t])).c,
      delivered:  (await db.getAsync('SELECT COALESCE(SUM(di.delivered_qty),0) c FROM delivery_items di JOIN deliveries d ON d.id=di.delivery_id WHERE d.date=?', [t])).c,
      returned:   (await db.getAsync('SELECT COALESCE(SUM(di.returned_qty),0) c FROM delivery_items di JOIN deliveries d ON d.id=di.delivery_id WHERE d.date=?', [t])).c,
    ...
    const recent = await db.allAsync(`
      ...
      GROUP BY d.id ORDER BY d.submitted_at DESC`, [t]);
    res.render('admin/dashboard', { stats, recent, today: t });
